...
description: |
  ...
  Also triggers on tool output likely to exceed 5KB, structured dumps, logs, builds, tests,
  API/JSON payloads, or unknown-size inspection.
  Subagent routing is handled automatically via PreToolUse hook.
---

# Context Mode: Default for Large or Unknown Output

## Default Rule

<context_mode_logic>
  <default_rule>
    Use context-mode for large, structured, repeated-query, or unknown-size output.
    Use shell directly for guaranteed-small observations, simple file/status checks, and mutations.
  </default_rule>
</context_mode_logic>

Shell is fine for:
- **Guaranteed-small observations**: `pwd`, `whoami`, `git status --short`, targeted `rg`, `Get-ChildItem` in a narrow folder
- **File mutations**: `mkdir`, `mv`, `cp`, `rm`, `touch`, `chmod`
- **Git writes**: `git add`, `git commit`, `git push`, `git checkout`, `git branch`, `git merge`
- **Navigation/process/package operations**: `cd`, `pwd`, `which`, `kill`, `npm install`, `pip install`
- **Simple output**: `echo`, `printf`

Use `ctx_execute` or `ctx_execute_file` for logs, builds/tests, broad diffs, large listings,
API/JSON output, cloud/container inventory, data files, or any output you need to query more than once.

If uncertain about output size, prefer context-mode. If the command is obviously tiny, use shell.
...
│
├── Command is guaranteed-small or mutates files/git/process/packages?
│   └── Use shell
│
├── Output likely >5KB, structured, repeated-query, or UNSURE?
...
│   └── Use filename parameter when snapshot/console/network output may be large, then:
...
| Run CLI that returns large data | `ctx_execute` | `gh pr list --json`, `aws s3 ls`, `kubectl get pods -A` |
| Run tests | `ctx_execute` | `npm test`, `pytest`, `go test ./...` |
| Large git output | `ctx_execute` | `git log --oneline -50`, `git diff HEAD~5` |
...
Use context-mode for these when output is large, structured, or unknown-size:
...
- **Git history**: "show recent commits", "git log", "what changed", "diff between branches" when output is not trivially bounded
...
- Use `source` parameter when multiple docs are indexed to avoid cross-source contamination
  - Partial match works: `source: "Node"` matches `"Node.js v22 CHANGELOG"`
- Use `queries` array for related search questions in one call:
  - `ctx_search(queries: ["transform pipe", "refine superRefine", "coerce codec"], source: "Zod")`
  - Avoid multiple separate ctx_search() calls for the same source/question set.

## External Documentation

- Use `ctx_fetch_and_index` for external docs you need to query/summarize; use direct web or official docs when only a small fact is needed.
...
5. **For small direct commands**: Use shell for guaranteed-small observations and mutations. Use context-mode for large/structured/unknown output.
...
7. **Use `filename` parameter** on Playwright tools for inspection or unknown-size output. Without it, large output can enter context.
...
<sandboxed_data_workflow>
  <critical_rule>
    When tool output may be large, use the 'filename' parameter.
...
**When a task involves Playwright snapshots, screenshots, or page inspection with unknown-size output, route through file → sandbox.**
...
> Use `filename` parameter when calling `browser_snapshot`, `browser_console_messages`, or `browser_network_requests` for inspection or unknown-size output.
...
- Using `gh pr list --json` via Bash without filtering → raw JSON in context. Use `ctx_execute` with `--jq` filter instead.
...
- Running noisy tests via Bash → full test output in context. Use `ctx_execute` to capture and summarize.
- Calling `browser_snapshot()` WITHOUT `filename` for inspection → 135K tokens may flood context. Use `browser_snapshot(filename: "/tmp/snap.md")`.
- Calling `browser_console_messages()` or `browser_network_requests()` WITHOUT `filename` for unknown-size output → entire output may flood context. Use the `filename` parameter.
- Passing large data to `ctx_index(content: ...)` → data enters context as a parameter. Use `ctx_index(path: ...)` to read server-side. The `content` parameter should only be used for small inline text you're composing yourself.
