...
## 1. Using execute for Guaranteed-Small Outputs
...
**Rule:** If the output is guaranteed small and not structured/reused, use Bash directly. Reserve `execute` for output likely `>5KB`, structured output, repeated-query work, or unknown-size inspection.
...
BAD workflow:
  ...
  → 10,000 lines consumed context for a question that needs only a small summary
...
- [ ] Output is likely `>5KB`, structured, repeated-query, or unknown-size; otherwise use Bash
