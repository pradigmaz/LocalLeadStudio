...
## 📦 Как собрать portable `.exe` (Windows)
...
2. Скрипт проверит Python 3.10+ и Node.js 18+ в `PATH`. Если одного из них нет или версия устарела, откроется официальная страница установки. После установки перезапустите `build-portable.bat`.

3. Готовый файл будет здесь: `electron/dist/Local Lead Studio 0.1.0.exe`.

Все зависимости проекта остаются рядом с исходниками: Python-пакеты и PyInstaller — в `backend/venv`, npm-пакеты — в `frontend/node_modules` и `electron/node_modules`, кэши сборки — в `.cache/`. В глобальные Python/npm-пакеты скрипт ничего не ставит. На ПК получателя этого portable `.exe` Python и Node.js не нужны.
